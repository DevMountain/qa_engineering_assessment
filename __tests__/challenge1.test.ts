...
describe("widget filter tests", () => {
  ...
  test("Evens and odds can be split", async () => {
    ...
    expect(results.evens).toContain("0,2,4,6,8,10");//switched this from odd to evens
    expect(results.odds).toContain("1,3,5,7,9");//switched this from evens to odd
  });
  test("The objects can be filtered", async () => {
    await widget.setObjectFilter("hairColor");//changed hair color to hairColor
